<template>
  ...
  <template v-else>
    <modal-delete
      ...
      question="Do you really want to delete task: "
      :title="task.task"
      @delete-clicked="onDeleteTaskClicked"
      @cancel-clicked="showModalDelete = false"
    />
    <t-modal
      :show="showAddPersonModal"
      @close-modal="showAddPersonModal = false"
    >
      <p>
...
export default {
  ...
  methods: {
    fetchPersonsForThisTask() {
      return db
        .get("personstasks?taskid=" + this.$route.params.taskid)
        .then((data) => {
  ...
  components: { TLoading, TButton, TModal, PersonList, ModalDelete },
};
</script>
